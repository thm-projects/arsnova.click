...
function formatSheet(ws, {responsesWithConfidenceValue, isCASRequired, columnsToFormat, leaderboardData, defaultStyles}) {
	...
	ws.cell(11, 1, (leaderboardData.length + 10), columnsToFormat).style(defaultStyles.attendeeEntryRowStyle);
...
function setSheetData(ws, {responsesWithConfidenceValue, translation, isCASRequired, questionGroup, questionGroupObject, hashtag, columnsToFormat, leaderboardData, numberOfAttendees}) {
	...
	ws.cell(9, nextColumnIndex++).string(TAPi18n.__("export.correct_questions", {lng: translation}));
	if (responsesWithConfidenceValue.length > 0) {
		ws.cell(9, nextColumnIndex++).string(TAPi18n.__("export.average_confidence", {lng: translation}));
	}
	ws.cell(9, nextColumnIndex++).string(TAPi18n.__("export.overall_response_time", {lng: translation}));
	ws.cell(9, nextColumnIndex++).string(TAPi18n.__("export.average_response_time", {lng: translation}));
...
export function generateSheet(wb, {hashtag, translation, defaultStyles}) {
	...
	formatSheet(ws, {responsesWithConfidenceValue, isCASRequired, columnsToFormat, leaderboardData, defaultStyles});
